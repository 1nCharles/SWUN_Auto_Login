...
def get_challenge():
    #登录认证第一步
    url = 'http://211.83.241.102/cgi-bin/get_challenge'
    headers = {
...
def srun_portal(username,password):
    #登录认证第二部
    url = 'http://211.83.241.102/cgi-bin/srun_portal'
    headers = {
    ...
    chkstr = token+username+token+hmd5+token+'2'+token+local_ip+token+'200'+token+'1'+token+info
    sha1 = hashlib.sha1()
...
if __name__ == '__main__':
    ...
    local_ip=GetLocalIPByPrefix('172.')
    temp=local_ip
    print(local_ip)
